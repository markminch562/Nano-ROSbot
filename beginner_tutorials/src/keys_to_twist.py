...
def ramped_vel(v_prev, v_target, t_prev, t_now, ramp_rate):
    #compute the maximum rate that velocity can stably be increased
...
def send_twist():
    global g_last_twist, g_vel_scales, g_target_twist, g_twist_pub, g_last_send_time,\
    g_vel_ramps
...
def fetch_param(name, default):
    if rospy.has_param(name):
        ...
        print "the paramiter [%s] is not defined Defaulting to %.3f" %(name, default)
        return default
...
if __name__ == '__main__':
    ...
    g_last_twist = Twist()
    g_vel_scales[1] = fetch_param('~angular_scale', 0.5)
    g_vel_scales[0] = fetch_param('~linear_scale', 0.5)
